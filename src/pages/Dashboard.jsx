...
export default function Dashboard() {
  ...
  const emptyKpis = useMemo(() => {
    return Object.fromEntries(Object.entries(kpis).map(([cat, metrics]) => [
      cat,
  ...
  const displayKpis = isToday ? kpis : (snapshot ? snapshot.kpis : emptyKpis)
  const displayRevenue = isToday ? revenue : (snapshot ? snapshot.revenue : revenue)
  const displayActiveProspects = isToday ? activeProspectsCount : (snapshot ? snapshot.activeProspects : activeProspectsCount)

  // ── render KPI category ──────────────────────────────────────────────────
  const renderCategory = (category, metrics, isHistorical) => {
    let icon = 'task'
    ...
    return (
      <section key={category} style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <GlowCard glowColor="teal" style={{
          ...
          border: isHistorical ? '1px solid rgba(224,169,109,0.15)' : '1px solid rgba(255,255,255,0.04)',
          borderTop: isHistorical ? '1px solid rgba(224,169,109,0.3)' : '1px solid rgba(255,255,255,0.15)',
          borderLeft: isHistorical ? '1px solid rgba(224,169,109,0.2)' : '1px solid rgba(255,255,255,0.12)',
                ...
                <div key={key} style={{ display: 'flex', flexDirection: 'column', gap: '16px', padding: '20px 24px', borderBottom: isLast ? 'none' : '1px solid rgba(255,255,255,0.04)' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', background: 'rgba(0,0,0,0.25)', border: '1px solid rgba(255,255,255,0.03)', borderTop: '1px solid rgba(0,0,0,0.5)', borderLeft: '1px solid rgba(0,0,0,0.3)', borderRadius: '10px', padding: '2px', boxShadow: 'inset 0 4px 8px rgba(0,0,0,0.3)' }}>
                      <button onClick={() => updateKpiValue(category, key, -1, selectedDay)} style={{ width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', color: 'rgba(180, 200, 200, 0.6)', cursor: 'pointer', borderRadius: '8px', transition: 'all 0.2s' }} className="glass-panel-hover">
                        <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>remove</span>
                      </button>
                      <input
                        type="number"
                        value={metric.value}
                        onChange={(e) => setKpiValueExact(category, key, e.target.value, selectedDay)}
                      ...
                      />
                      <button onClick={() => updateKpiValue(category, key, 1, selectedDay)} style={{ width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', color: '#52b788', cursor: 'pointer', borderRadius: '8px', transition: 'all 0.2s' }} className="glass-panel-hover">
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      {!isToday && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 18px', background: 'rgba(224,169,109,0.08)', border: '1px solid rgba(224,169,109,0.2)', borderRadius: '12px', fontSize: '13px', color: '#e0a96d' }}>
          ...
          Editing historical data for <strong>{selectedDay}</strong>. Switch back to{' '}
